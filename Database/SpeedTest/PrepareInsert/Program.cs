...
internal static class Program {
	...
	private static int DeleteRecordList(SqlCommand command, IEnumerable<DsvRecord> records) {
		...
		var parameter = command.Parameters.Add("Field001", SqlDbType.VarChar, 100);
		command.Prepare();
	...
	private static int InsertRecordList(SqlCommand command, IEnumerable<DsvRecord> records) {
		...
		var length = 0;
		foreach (var record in records) {
			length = Math.Max(length, record.Count);
		}
		// SQL文作成
		...
		command.Parameters.Clear();
		for (var index = 1; index <= length; index ++) {
			command.Parameters.Add($"Field{index:000}", SqlDbType.VarChar);
		}
		foreach (var record in records) {
			for (var index = 1; index <= length; index ++) {
				if (record.TryGetValue($"Field{index:000}", out var data)) {
					command.Parameters[index - 1].Value = data;
				} else {
					command.Parameters[index - 1].Value = DBNull.Value;
				}
			}
			result += ExecuteNonQuery(command);
		}
